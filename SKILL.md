---
name: the-larsson-binance-trader
description: The Larsson Binance spot trading bot with LLM-powered market analysis and auto-discovery. Supports momentum, mean reversion, and DCA strategies with news sentiment integration. Use when user wants to trade on Binance, set up automated crypto trading, or manage the trading bot. Requires Binance API credentials.
---
...
1. **Source of Truth**: Binance API is always checked first
2. **Dust Filter**: Positions under $1 USD are ignored (staking rewards, etc.)
3. **Sync**: If you sell manually on Binance, bot treats local position as "closed"
4. **Logging**: `trades.jsonl` tracks history but is NOT used for trading decisions
...
1. **Scans** top coins by volume
2. **Analyzes** technical indicators (EMA, RSI, volume)
3. **Checks** news sentiment for each coin
4. **Scores** opportunities (0-1 scale)
5. **Auto-adds** pairs with score ≥ DISCOVERY_MIN_SCORE
