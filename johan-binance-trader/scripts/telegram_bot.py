...
def handle_command(cmd):
    ...
    if cmd == '/status':
        ...
        # Find the trader process (check for trader_continuous.sh or trader.py)
        try:
            # First try to find trader_continuous.sh
            r = subprocess.run(['pgrep', '-f', 'trader_continuous'],
                              capture_output=True, text=True, timeout=5)
            if r.stdout.strip():
                pid = r.stdout.strip().split('\n')[0]
                r2 = subprocess.run(['ps', '-p', pid, '-o', 'etime=,cmd='],
                                   capture_output=True, text=True, timeout=5)
                if r2.returncode == 0:
                    uptime = r2.stdout.strip().split()[0] if r2.stdout.strip() else "Unknown"
                else:
                    uptime = "Unknown"
            else:
                # Try trader.py as fallback
                r = subprocess.run(['pgrep', '-f', 'trader.py'],
                                  capture_output=True, text=True, timeout=5)
                if r.stdout.strip():
                    pid = r.stdout.strip().split('\n')[0]
